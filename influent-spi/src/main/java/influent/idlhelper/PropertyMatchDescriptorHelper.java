...
public class PropertyMatchDescriptorHelper extends FL_PropertyMatchDescriptor {
  ...
  public static String toJson(Map<String, List<FL_PropertyMatchDescriptor>> map)
      throws IOException {
  ...
  public static Map<String, List<FL_PropertyMatchDescriptor>> mapFromJson(String json)
      throws IOException {
   ...
   *
   * @param terms the terms to represent
   * @return an FL_*Range Object - either a singleton or list
   */
  public static Object rangeFromBasicTerms(String terms) {

    // if quoted, return as one term
    if (terms.charAt(0) == '"') {
      int end = terms.length() - (terms.charAt(terms.length() - 1) == '"' ? 1 : 0);
  ...
  /**
   * Returns true if the match descriptor is exclusive, accounting for both the include property and
   * the NOT constraint.
   *
   * <p>Match descriptors have a NOT constraint which is redundant with the include boolean for
   * searches other than pattern searches. Here we interpret "NOT exclude" as "NOT/exclude", since
   * otherwise the criteria would have no effect at all.
   *
   * @param descriptor The match specification
