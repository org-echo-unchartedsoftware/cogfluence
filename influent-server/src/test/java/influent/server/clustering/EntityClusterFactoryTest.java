...
public class EntityClusterFactoryTest {
  ...
  public void setup() {
    Properties pMgr =
        new Properties() {
          ...
          public void init() {
            values.put(
                "entity.clusterer.clusterproperties",
                "TYPE:type-dist,GEO:location-dist,AvTransAmnt:avTran-dist");
          }
    ...
    IdGenerator idGen =
        new IdGenerator() {
          ...
          public String nextId() {
            return InfluentId.fromNativeId(
                    InfluentId.CLUSTER, "cluster", UUID.randomUUID().toString())
                .getInfluentId();
          }
        };
    ...
    entityB =
        createEntity(
            "entityB", FL_EntityTag.ANONYMOUS, "Vancouver", "CAN", 49.2500, -123.1000, 200.0, 5, 7);
    entityC =
        createEntity(
            "entityC", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 300.0, 11, 13);
    entityD =
        createEntity(
            "entityD", FL_EntityTag.ACCOUNT, "LA", "USA", 34.0522342, -118.2436849, 400.0, 17, 19);
    ...
    clusterSummaryA =
        createClusterSummary(
            "clusterSummaryA", locationDist, typeDist, avTranDist, 700, 3000, 7000);
  }
  ...
  private FL_Entity createEntity(
    ...
    props.add(
        new PropertyHelper(
            "AvTransAmnt", "AvTransAmnt", avTransAmnt, Arrays.asList(FL_PropertyTag.STAT)));
    FL_GeoData geoData =
  ...
  public void testCreateMutableClusterWithSubCluster() {
    ...
    FL_Cluster cluster =
        clusterFactory.toCluster(Arrays.asList(entityA, entityB), Arrays.asList(subcluster));
  ...
  public void testCreateMutableClusterWithSubClusterSummary() {
    FL_Cluster cluster =
        clusterFactory.toCluster(Arrays.asList(entityA, entityB), Arrays.asList(clusterSummaryA));
  ...
  public void testCreateOwnerCluster() {
    FL_Cluster owner =
        clusterFactory.toAccountOwnerSummary(
            entityA, Arrays.asList(entityB, entityC), new ArrayList<FL_Cluster>(0));

    Assert.assertEquals(owner.getMembers().size(), 2);
    Assert.assertEquals(owner.getSubclusters().size(), 0);
    Assert.assertEquals(
        owner.getTags(), Arrays.asList(FL_EntityTag.CLUSTER, FL_EntityTag.ACCOUNT_OWNER));
  ...
  public void testCreateOwnerClusterWithSubCluster() {
    FL_Cluster subcluster =
        clusterFactory.toCluster(Arrays.asList(entityD), new ArrayList<FL_Cluster>(0));
    FL_Cluster owner =
        clusterFactory.toAccountOwnerSummary(
            entityA, Arrays.asList(entityB, entityC), Arrays.asList(subcluster));
