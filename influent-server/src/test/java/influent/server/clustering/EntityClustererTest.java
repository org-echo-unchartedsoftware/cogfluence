...
public class EntityClustererTest {
  ...
  private void assertClusterEqualsUnordered(
      String clusterId, String value, ClusterContext context) {
    FL_Cluster cluster = context.clusters.get(clusterId);
    if (cluster == null) Assert.fail("Invalid cluster id");
    ...
    if (!expectedPrefix.equals(actualPrefix)) {
      Assert.fail(
          "Cluster prefix mismatch. Expected: " + expectedPrefix + " but was: " + actualPrefix);
    }

    // Extract subclusters from both strings
    String expectedSubclusters =
        value.substring(value.lastIndexOf('[') + 1, value.lastIndexOf(']'));
    String actualSubclusters =
        actualClusterString.substring(
            actualClusterString.lastIndexOf('[') + 1, actualClusterString.lastIndexOf(']'));
    ...
    if (expectedSubclusters.isEmpty() || actualSubclusters.isEmpty()) {
      Assert.fail(
          "One has subclusters while the other doesn't. Expected: "
              + expectedSubclusters
              + " but was: "
              + actualSubclusters);
    }
    ...
    java.util.Set<String> expectedSet =
        new java.util.HashSet<String>(java.util.Arrays.asList(expectedSubclusters.split(",")));
    java.util.Set<String> actualSet =
        new java.util.HashSet<String>(java.util.Arrays.asList(actualSubclusters.split(",")));
  ...
  public void testClusterByTypeMax() {
    ...
    assertClusterEqualsUnordered(
        "c.cluster.1", "{c.cluster.1:[][c.cluster.4,c.cluster.3]}", context);
    assertClusterEquals("c.cluster.2", "{c.cluster.2:[entityB][]}", context);
  ...
  public void testClusterByMaxMultiLevel() {
    ...
    assertClusterEqualsUnordered(
        "c.cluster.3", "{c.cluster.3:[][c.cluster.6,c.cluster.5]}", context);
  ...
  public void testClusterByNumberIncremental() {
    ...
    assertClusterEqualsUnordered(
        "c.cluster.1", "{c.cluster.1:[][c.cluster.6,c.cluster.5]}", context);
  ...
  public void testClusterByNumberMax() {
    ...
    assertClusterEqualsUnordered(
        "c.cluster.1", "{c.cluster.1:[][c.cluster.2,c.cluster.3]}", context);
    ...
    assertClusterEqualsUnordered(
        "c.cluster.5", "{c.cluster.5:[][c.cluster.6,c.cluster.7]}", context);
