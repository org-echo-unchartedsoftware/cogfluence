...
public class EntityClustererTest {
  ...
  private GeneralEntityClusterer createClusterer(
      final String clusterProperties, final int maxClusterSize) {
    Properties pMgr =
        new Properties() {
    ...
    IdGenerator idGen =
        new IdGenerator() {
          ...
          public String nextId() {
            return InfluentId.fromNativeId(InfluentId.CLUSTER, "cluster", (++id).toString())
                .getInfluentId();
          }
        };
  ...
  private FL_Entity createEntity(
      String name,
      FL_EntityTag tag,
      String location,
      String cc,
    ...
    FL_GeoData geoData =
        FL_GeoData.newBuilder().setText(location).setLat(lat).setLon(lon).setCc(cc).build();
    props.add(
        FL_Property.newBuilder()
            ...
            .setUncertainty(null)
            .build());
    return new EntityHelper(name, name, tag.name(), tag, props);
  }

  @SuppressWarnings("unused")
  private FL_Cluster createClusterSummary(
      String name,
    ...
    props.add(
        new PropertyHelper(
            ...
            Arrays.asList(FL_PropertyTag.OUTFLOWING, FL_PropertyTag.AMOUNT, FL_PropertyTag.USD)));
    props.add(
        new PropertyHelper("count", "count", count, FL_PropertyType.INTEGER, FL_PropertyTag.STAT));
    ...
    for (String cc : locationDist.keySet()) {
      FL_GeoData geo =
          FL_GeoData.newBuilder().setText(null).setLat(null).setLon(null).setCc(cc).build();
    ...
    return new ClusterHelper(
        InfluentId.fromNativeId(InfluentId.CLUSTER_SUMMARY, "cluster", name).getInfluentId(),
        name,
  ...
  private String clusterToString(FL_Cluster cluster) {
    return "{"
        ...
        + "]["
        + StringUtils.join(cluster.getSubclusters(), ",")
        + "]}";
  }
  ...
  private void assertClusterEqualsUnordered(String clusterId, String value, ClusterContext context) {
    FL_Cluster cluster = context.clusters.get(clusterId);
    if (cluster == null) Assert.fail("Invalid cluster id");
    
    // Get actual cluster string
    String actualClusterString = clusterToString(cluster);
    
    // Extract prefix (cluster ID and members part)
    String expectedPrefix = value.substring(0, value.lastIndexOf('['));
    String actualPrefix = actualClusterString.substring(0, actualClusterString.lastIndexOf('['));
    
    if (!expectedPrefix.equals(actualPrefix)) {
      Assert.fail("Cluster prefix mismatch. Expected: " + expectedPrefix + " but was: " + actualPrefix);
    }
    
    // Extract subclusters from both strings
    String expectedSubclusters = value.substring(value.lastIndexOf('[') + 1, value.lastIndexOf(']'));
    String actualSubclusters = actualClusterString.substring(actualClusterString.lastIndexOf('[') + 1, actualClusterString.lastIndexOf(']'));
    
    if (expectedSubclusters.isEmpty() && actualSubclusters.isEmpty()) {
      return; // Both empty, they match
    }
    
    if (expectedSubclusters.isEmpty() || actualSubclusters.isEmpty()) {
      Assert.fail("One has subclusters while the other doesn't. Expected: " + expectedSubclusters + " but was: " + actualSubclusters);
    }
    
    // Convert to sets and compare (order-independent)
    java.util.Set<String> expectedSet = new java.util.HashSet<String>(java.util.Arrays.asList(expectedSubclusters.split(",")));
    java.util.Set<String> actualSet = new java.util.HashSet<String>(java.util.Arrays.asList(actualSubclusters.split(",")));
    
    Assert.assertEquals("Subclusters do not match", expectedSet, actualSet);
  }
  ...
  public void testClusterByType() {
    ...
    entities.add(
        createEntity(
            "entityB", FL_EntityTag.ANONYMOUS, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity(
            "entityC", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 11, 13));
  ...
  public void testClusterByTypeMax() {
    ...
    entities.add(
        createEntity(
            "entityD", FL_EntityTag.ACCOUNT, "LA", "USA", 34.0522342, -118.2436849, 17, 19));
    ...
    assertClusterEqualsUnordered("c.cluster.1", "{c.cluster.1:[][c.cluster.4,c.cluster.3]}", context);
    assertClusterEquals("c.cluster.2", "{c.cluster.2:[entityB][]}", context);
  ...
  public void testClusterByMaxMultiLevel() {
    ...
    entities.add(
        createEntity(
            "entityC", FL_EntityTag.ANONYMOUS, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity(
            "entityD", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 11, 13));
    entities.add(
        createEntity(
            "entityE", FL_EntityTag.ACCOUNT, "LA", "USA", 34.0522342, -118.2436849, 17, 19));
    entities.add(
        createEntity(
            "entityF", FL_EntityTag.ACCOUNT, "LA", "USA", 34.0522342, -118.2436849, 17, 19));
    entities.add(
        createEntity(
            "entityG", FL_EntityTag.ACCOUNT, "LA", "USA", 34.0522342, -118.2436849, 17, 19));
    ...
    assertClusterEqualsUnordered("c.cluster.3", "{c.cluster.3:[][c.cluster.6,c.cluster.5]}", context);
  ...
  public void testClusterByLabel() {
    ...
    entities.add(
        createEntity(
            "Adamn", FL_EntityTag.ACCOUNT, "Toronto", "CAN", 43.653226, -79.38318429999998, 2, 3));
    entities.add(
        createEntity("Alf", FL_EntityTag.ANONYMOUS, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity("Zulu", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 11, 13));
    ...
    assertClusterEquals(
        "c.cluster.1", "{c.cluster.1:[Adamn,Alf][]}", context); // cluster by alpha "A-M"
  ...
  public void testClusterByLabelFingerprint() {
    ...
    entities.add(
        createEntity(
            "Amandå", FL_EntityTag.ACCOUNT, "Toronto", "CAN", 43.653226, -79.38318429999998, 2, 3));
    entities.add(
        createEntity("Bob", FL_EntityTag.ANONYMOUS, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity(
            "Amanda", FL_EntityTag.ANONYMOUS, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity("Alf", FL_EntityTag.ANONYMOUS, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity("Zulu", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 11, 13));
    ...
    assertClusterEqualsUnordered(
        "c.cluster.1",
        "{c.cluster.1:[][c.cluster.4,c.cluster.3]}",
        context); // cluster by alpha "A-M"
    assertClusterEquals("c.cluster.2", "{c.cluster.2:[Zulu][]}", context); // cluster by alpha "N-Z"
    assertClusterEquals("c.cluster.4", "{c.cluster.4:[Bob][]}", context); // cluster by alpha "B"
    assertClusterEquals(
        "c.cluster.3", "{c.cluster.3:[][c.cluster.5]}", context); // cluster by alpha "A"
    assertClusterEquals(
        "c.cluster.5",
        "{c.cluster.5:[][c.cluster.6]}",
        context); // cluster by fuzzy matching   <-- Strange
    assertClusterEqualsUnordered(
        "c.cluster.6",
        "{c.cluster.6:[][c.cluster.8,c.cluster.7]}",
        context); // cluster by fingerprint

    FL_Cluster leaf1 = context.clusters.get("c.cluster.7");

    if (leaf1.getMembers().size() == 1) {
      assertClusterEquals(
          "c.cluster.7", "{c.cluster.7:[Alf][]}", context); // cluster by fingerprint
      assertClusterEquals(
          "c.cluster.8", "{c.cluster.8:[Amandå,Amanda][]}", context); // cluster by fingerprint
    } else {
      assertClusterEquals(
          "c.cluster.8", "{c.cluster.8:[Alf][]}", context); // cluster by fingerprint
      assertClusterEquals(
          "c.cluster.7", "{c.cluster.7:[Amandå,Amanda][]}", context); // cluster by fingerprint
    }
  }
  ...
  public void testClusterByLabelMax() {
    ...
    entities.add(
        createEntity(
            "Adan", FL_EntityTag.ACCOUNT, "Toronto", "CAN", 43.653226, -79.38318429999998, 2, 3));
    entities.add(
        createEntity("Alf", FL_EntityTag.ANONYMOUS, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity(
    ...
    assertClusterEquals(
        "c.cluster.1", "{c.cluster.1:[][c.cluster.3]}", context); // cluster by alpha "A-M"
    assertClusterEquals("c.cluster.2", "{c.cluster.2:[Zulu][]}", context); // cluster by alpha "N-Z"
    assertClusterEquals(
        "c.cluster.3", "{c.cluster.3:[][c.cluster.4]}", context); // cluster by alpha "A"
    assertClusterEquals(
        "c.cluster.4",
        "{c.cluster.4:[][c.cluster.8,c.cluster.9]}",
        context); // cluster by alpha "A"
    ...
    Assert.assertTrue(
        (leaf1.getMembers().size() == 1 && leaf2.getMembers().size() == 2)
  ...
  public void testClusterByLabelIncremental() {
    ...
    assertClusterEquals("c.cluster.2", "{c.cluster.2:[Zulu][]}", context); // cluster by alpha "N-Z"
    assertClusterEquals(
        "c.cluster.3", "{c.cluster.3:[Amandå,Alf][]}", context); // cluster by alpha "A"
    assertClusterEquals("c.cluster.4", "{c.cluster.4:[Bob][]}", context); // cluster by alpha "B"
    ...
    assertClusterEqualsUnordered(
        "c.cluster.5",
        "{c.cluster.5:[][c.cluster.6,c.cluster.7]}",
        context); // cluster by fuzzy matching

    FL_Cluster leaf1 = context.clusters.get("c.cluster.6");

    if (leaf1.getMembers().size() == 1) {
      assertClusterEquals(
          "c.cluster.6", "{c.cluster.6:[Alf][]}", context); // cluster by fingerprint
      assertClusterEquals(
          "c.cluster.7", "{c.cluster.7:[Amandå,Amanda][]}", context); // cluster by fingerprint
    } else {
      assertClusterEquals(
          "c.cluster.7", "{c.cluster.7:[Alf][]}", context); // cluster by fingerprint
      assertClusterEquals(
          "c.cluster.6", "{c.cluster.6:[Amandå,Amanda][]}", context); // cluster by fingerprint
    }
  }
  ...
  public void testClusterByNumberSingleCluster() {
    ...
    entities.add(
        createEntity(
            ...
            3));
    entities.add(
        createEntity("Zulu", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 500, 13));
  ...
  public void testClusterByNumberIncremental() {
    ...
    entities.add(
        createEntity(
            ...
            3));
    entities.add(
        createEntity("Bob", FL_EntityTag.ACCOUNT, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity("Zulu", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 500, 13));
    ...
    assertClusterEqualsUnordered("c.cluster.1", "{c.cluster.1:[][c.cluster.6,c.cluster.5]}", context);
    ...
    entities =
        Collections.singletonList(
            createEntity(
                "Alf", FL_EntityTag.ACCOUNT, "Vancouver", "CAN", 49.2500, -123.1000, 8, 7));
    context.addEntities(entities);
    context = clusterer.clusterEntities(entities, context);
    ...
    if (leaf1.getMembers().size() == 1) {
      assertClusterEquals("c.cluster.5", "{c.cluster.5:[Amanda][]}", context);
      assertClusterEqualsUnordered("c.cluster.6", "{c.cluster.6:[][c.cluster.8,c.cluster.9]}", context);
      assertClusterEquals("c.cluster.8", "{c.cluster.8:[Zulu][]}", context);
      assertClusterEquals("c.cluster.9", "{c.cluster.9:[Bob,Alf][]}", context);
    } else {
      assertClusterEquals("c.cluster.6", "{c.cluster.6:[Amanda][]}", context);
      assertClusterEqualsUnordered("c.cluster.5", "{c.cluster.5:[][c.cluster.8,c.cluster.9]}", context);
      assertClusterEquals("c.cluster.8", "{c.cluster.8:[Zulu][]}", context);
      assertClusterEquals("c.cluster.9", "{c.cluster.9:[Bob,Alf][]}", context);
    }
  }
  ...
  public void testClusterByNumberMax() {
    ...
    entities.add(
        createEntity("Bob", FL_EntityTag.ACCOUNT, "Vancouver", "CAN", 49.2500, -123.1000, 5, 7));
    entities.add(
        createEntity("Alf", FL_EntityTag.ACCOUNT, "Vancouver", "CAN", 49.2500, -123.1000, 8, 7));
    entities.add(
        createEntity("Zulu", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 6, 13));
    ...
    assertClusterEqualsUnordered("c.cluster.1", "{c.cluster.1:[][c.cluster.2,c.cluster.3]}", context);
    ...
    assertClusterEqualsUnordered("c.cluster.5", "{c.cluster.5:[][c.cluster.6,c.cluster.7]}", context);
  ...
  public void testClusterByGeoMultiLevel() {
    ...
    entities.add(
        createEntity(
            ...
            3));
    entities.add(
        createEntity("Bob", FL_EntityTag.ACCOUNT, "Vancouver", "CAN", 49.2827, -123.1207, 5, 7));
    entities.add(
        createEntity("Steve", FL_EntityTag.ACCOUNT, "Victoria", "CAN", 48.4222, -123.3657, 5, 7));
    entities.add(
        createEntity("Dan", FL_EntityTag.ACCOUNT, "Montreal", "CAN", 45.5017, -73.5673, 5, 7));
    entities.add(
        createEntity("Alf", FL_EntityTag.ACCOUNT, "London", "GBR", 51.5072, -0.1275, 8, 7));
    entities.add(
        createEntity("Zulu", FL_EntityTag.ACCOUNT, "NYC", "USA", 40.7143528, -74.0059731, 6, 13));
    ...
    assertClusterEquals(
        "c.cluster.3", "{c.cluster.3:[][c.cluster.4,c.cluster.5]}", context); // USA and CAN
    assertClusterEquals("c.cluster.5", "{c.cluster.5:[Zulu][]}", context); // USA
    assertClusterEquals(
        "c.cluster.4", "{c.cluster.4:[][c.cluster.6,c.cluster.7,c.cluster.8]}", context); // CAN
    assertClusterEquals("c.cluster.6", "{c.cluster.6:[Amanda][]}", context); // Toronto
    assertClusterEquals(
        "c.cluster.7", "{c.cluster.7:[Bob,Steve][]}", context); // Vancouver and Victoria
  ...
  public void testClusterByGeoMissingProp() {
    ...
    assertClusterEquals("c.cluster.1", "{c.cluster.1:[Alf][]}", context); // Europe
    assertClusterEquals("c.cluster.2", "{c.cluster.2:[][c.cluster.4]}", context); // Unknown
    assertClusterEquals("c.cluster.3", "{c.cluster.3:[Zulu][]}", context); // North America
    assertClusterEquals("c.cluster.4", "{c.cluster.4:[][c.cluster.5]}", context); // CAN
    assertClusterEquals(
        "c.cluster.5", "{c.cluster.5:[][c.cluster.6,c.cluster.7,c.cluster.8]}", context); // CAN
    assertClusterEquals("c.cluster.6", "{c.cluster.6:[Amanda][]}", context); // Toronto
    assertClusterEquals(
        "c.cluster.7", "{c.cluster.7:[Bob,Steve][]}", context); // Vancouver and Victoria
    assertClusterEquals("c.cluster.8", "{c.cluster.8:[Dan][]}", context); // Montreal
