...
public class SQLBuilderTest {
  ...
  public void testSelectDistinctClause() {
    MSSQLBuilder builder = new MSSQLBuilder();
    SQLSelect sql = builder.select().distinct().column("col1").from("myTable");
  ...
  public void testSelectTopNClause() {
    ...
    sql = builder.select().top(10).column("col1").from("myTable");
    Assert.assertEquals("SELECT TOP 10 col1 FROM myTable", sql.build());

    sql = builder.select().topPercent(10.5).column("col1").from("myTable");
  ...
  public void testSelectColumnClause() {
    ...
    sql = builder.select().column("col1", "blah", SQLFunction.Count).from("myTable");
    ...
    sql =
        builder
            ...
            .from("myTable");
    Assert.assertEquals(
        "SELECT COUNT(DISTINCT col1) AS blah,LCASE(col2),COUNT(*) FROM myTable", sql.build());
  }
  ...
  public void testSelectFromClause() {
    ...
    sql = builder.select().column("col1").from("myTable", "m");
    Assert.assertEquals("SELECT col1 FROM myTable AS m", sql.build());

    sql = builder.select().column("col1").from(builder.from().table("myTable").as("m"));
    Assert.assertEquals("SELECT col1 FROM myTable AS m", sql.build());
    ...
    sql =
        builder
            .select()
            .column("col1")
            .from(builder.from().fromQuery(builder.select().from("myTable2", "m2")).as("m"));
  ...
  public void testSelectWhereClause() {
    ...
    sql =
        builder
            ...
            .column("col1")
            .from("myTable", "m")
            .where(builder.filter().column("col1").eq(5));
    ...
    sql =
        builder
            ...
            .column("col1")
            .from("myTable")
            .where(builder.filter().column("col1").lessThan(5).not());
    ...
    sql =
        builder
            ...
            .column("col1")
            .from("myTable")
            .where(builder.group(builder.filter().column("col1").lessThanEq(5).not()));
    Assert.assertEquals("SELECT col1 FROM myTable WHERE (col1 NOT <= 5)", sql.build());
    ...
    sql =
        builder
            ...
            .column("col1")
            .from("myTable")
            .where(builder.not(builder.filter().column("col1").greaterThan(5).not()));
    ...
    sql =
        builder
            ...
            .column("col1")
            .from("myTable")
            .where(builder.group(builder.filter().column("col1").greaterThanEq(5)).not());
    ...
    sql =
        builder
            ...
            .where(
                builder.or(
                    builder.filter().column("col1").like("'%s'").not(),
                    builder.filter().column("col2").notEq("'text'")));
    Assert.assertEquals(
        "SELECT col1,col2 FROM myTable WHERE (col1 NOT LIKE '%s') OR (col2 != 'text')",
        sql.build());
    ...
    sql =
        builder
            ...
            .where(
                builder
                    .and(
                        builder.filter().column("col1").between(5, 10),
                        builder
                            .filter()
                            .column("col2")
                    ...
                    .not());
    Assert.assertEquals(
        "SELECT col1,col2 FROM myTable WHERE NOT((col1 BETWEEN 5 AND 10) AND (col2 IN ('a_name')))",
        sql.build());
  ...
  public void testSelectGroupByClause() {
    ...
    sql = builder.select().column("col1", "c", SQLFunction.Count).from("myTable").groupBy("c");
    ...
    sql =
        builder
            ...
            .groupBy("a");
    Assert.assertEquals(
        "SELECT COUNT(col1) AS c,AVG(col2) AS a FROM myTable GROUP BY c,a", sql.build());
  }
  ...
  public void testSelectOrderByClause() {
    ...
    sql = builder.select().column("col1").column("col2").from("myTable").orderBy("col1");
    Assert.assertEquals("SELECT col1,col2 FROM myTable ORDER BY col1 ASC", sql.build());

    sql = builder.select().column("col1").column("col2").from("myTable").orderBy("col2", true);
    Assert.assertEquals("SELECT col1,col2 FROM myTable ORDER BY col2 ASC", sql.build());

    sql = builder.select().column("col1").column("col2").from("myTable").orderBy("col2", false);
  ...
  public void testSelectGroupByOrderByClause() {
    ...
    sql =
        builder
            ...
            .orderBy("col2");
    Assert.assertEquals(
        "SELECT col1,col2 FROM myTable GROUP BY col1 ORDER BY col2 ASC", sql.build());
  }
  ...
  public void testSelectJoinClause() {
    ...
    sql =
        builder
            ...
            .column("col1")
            .column("col2")
            .from(builder.from().table("myTable").as("m"))
            .join(builder.innerJoin().table("myTable2").as("m2").on("m.col1=m.col2"));
    Assert.assertEquals(
        "SELECT col1,col2 FROM myTable AS m INNER JOIN myTable2 AS m2 ON m.col1=m.col2",
        sql.build());
  }
  ...
  public void testSelectVarReplacement() {
    ...
    try {
      sql =
          builder
              ...
              .column("col2")
              .from("myTable")
              .where(builder.filter().column("col1").eq(builder.lazyParam("lazy1Value")));
      sql.build();
    ...
    Assert.assertEquals(
        "SELECT col1,col2 FROM myTable WHERE (col1 = 7) AND (col2 < 10)", sql.build(replacements));
    ...
    sql =
        builder
            ...
            .column("col1")
            .from("myTable")
            .where(builder.filter().column("col1").like(builder.lazyParam("lazy3Value")));
    Assert.assertEquals(
        "SELECT col1 FROM myTable WHERE col1 LIKE 'blah%'", sql.build(replacements));
  }
}
