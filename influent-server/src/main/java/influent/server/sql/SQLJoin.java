...
/**
 * Creates a SQL JOIN statement and binds two tables together using a given ON clause. This is very
 * similar to a {@link SQLFrom} clause.<br>
 * Selecting the different types of join statements is done by using the {@link SQLBuilder}.
 *
 * @author cregnier
 */
public interface SQLJoin {
   ...
   *
   * @param name The name of an existing table in the database
   * @return
   */
   ...
   *
   * @param alias The name of the alias that will be used in the SQL statement.
   * @return
   */
   ...
   *
   * @param fromQuery A subquery to select results from.
   * @return
   */
   ...
   *
   * @param onClause A string that specifies which table columns to join together.
   * @return
   */
