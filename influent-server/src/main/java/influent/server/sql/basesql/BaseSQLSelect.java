...
public class BaseSQLSelect implements SQLSelect {
  ...
  /**
   * This represents the intermediate results for each of the builder functions needed to build the
   * final sql statement.
   *
   * @author cregnier
   */
        ...
        for (Entry<String, Integer> entry : o.indices.entrySet()) {
          ...
          if (oldVal != null) {
            throw new SQLBuilderException(
                "The named parameter '" + entry.getKey() + "' was used multiple times");
          }
        }
      }
    }
  ...
  public SQLSelect join(SQLJoin joinClause) {
    if (!(joinClause instanceof BaseSQLJoin)) {
      throw new SQLBuilderException(
          "Unexpected from clause is an unexpected type: "
              + joinClause.getClass()
              + ". Should be "
              + BaseSQLJoin.class);
    }
  ...
  public SQLSelect union(SQLSelect select) {
    if (!(select instanceof BaseSQLSelect)) {
      throw new SQLBuilderException(
          "Union select clause is an unexpected type: "
              + select.getClass()
              + ". Should be "
              + BaseSQLSelect.class);
    }
  ...
  public SQLSelect unionAll(SQLSelect select) {
    if (!(select instanceof BaseSQLSelect)) {
      throw new SQLBuilderException(
          "Union all select clause is an unexpected type: "
              + select.getClass()
              + ". Should be "
              + BaseSQLSelect.class);
    }
  ...
  public String build(Map<String, Object> paramValues) {
    ...
    if (buildResult != null && (buildResult.indices != null && buildResult.indices.size() > 0)) {
      throw new SQLBuilderException(
          "Tried to build an unprepared statement but there were unfulfilled params.");
    }
  ...
  public SQLPreparedResult buildPrepared(Map<String, Object> paramValues) {
    ...
    result.indices =
        (buildResult != null && buildResult.indices != null)
            ? buildResult.indices
  ...
  private BuildResult buildSelectClause(
      BaseSQLSelect selectClause, StringBuilder strb, Map<String, Object> paramValues) {
    BuildResult result = new BuildResult();

    strb.append("SELECT ");
    if (usesTop() && topCount != null && topCount.doubleValue() > 0) {
        ...
        case Mid:
          {
            if (col.fnParams == null || col.fnParams.length < 1) {
              ...
              throw new SQLBuilderException(
                  "column MID(" + col.name + ") does not have enough parameters");
            }
        ...
        case Round:
          {
            if (col.fnParams == null || col.fnParams.length < 1) {
              // throw error, not enough params
              throw new SQLBuilderException(
                  "column ROUND(" + col.name + ") does not have enough parameters");
            }
        ...
        case Format:
          {
            if (col.fnParams == null || col.fnParams.length < 1) {
              // throw error, not enough params
              throw new SQLBuilderException(
                  "column FORMAT(" + col.name + ") does not have enough parameters");
            }
  ...
  private BuildResult buildWhereClause(
      SQLFilter filterClause, StringBuilder strb, Map<String, Object> paramValues) {
    StringBuilder filterBuf = new StringBuilder();
    BuildResult res = buildFilterClause(filterClause, filterBuf, paramValues);

    // since the where filter may turn out to be empty, we check if there's text to add before
  ...
  private BuildResult buildFilterClause(
      SQLFilter filterClause, StringBuilder strb, Map<String, Object> paramValues) {
  ...
  private BuildResult buildFilterColumnClause(
      BaseSQLFilterColumn columnFilter, StringBuilder strb, Map<String, Object> paramValues) {
    BuildResult result = new BuildResult();
    ...
    return (result.indices != null)
        ? result
        : null; // only return the result if we actually added anything to it
  }

  private BuildResult buildFilterGroupClause(
      BaseSQLFilterGroup groupFilter, StringBuilder strb, Map<String, Object> paramValues) {
    BuildResult result = null;
    ...
    for (int i = 0; i < numFilters; ++i) {
      ...
      if (filterResult != null) {
        if (result == null) result = filterResult;
        else result.addResult(filterResult);
      }

      if (subfilterBuilder.length() > 0) subfilterBuilders.add(subfilterBuilder);
    }
    ...
    if (actualFilters > 0) {
      ...
      if (actualFilters > 1) strb.append("(");
      ...
      for (int i = 0; i < actualFilters - 1; ++i) {
        ...
        switch (groupFilter.getFilterGroupType()) {
          ...
          default:
            throw new SQLBuilderException(
                "Unknown group filter type: " + groupFilter.getFilterGroupType());
        }
      }
      ...
      if (groupFilter.isNegated()
          || groupFilter.getFilterGroupType().equals(FilterGroupType.GROUP)) {
  ...
  private BuildResult buildFromSelectClause(
      String tableName,
      BaseSQLSelect selectQuery,
      String alias,
      StringBuilder strb,
    ...
    if (tableName != null && selectQuery != null) {
      ...
      throw new SQLBuilderException(
          "From query (" + tableName + ") should not contain a table name and a select query");
    }
  ...
  private BuildResult buildFromClause(
      BaseSQLFrom fromClause, StringBuilder strb, Map<String, Object> paramValues) {
    if (fromClause == null) {
      throw new SQLBuilderException("from clause should not be null");
    }

    strb.append("FROM ");
    return buildFromSelectClause(
        fromClause.getTableName(),
        fromClause.getSelectQuery(),
        fromClause.getAlias(),
        strb,
        paramValues);
  }

  private BuildResult buildJoinClause(
      BaseSQLJoin joinClause, StringBuilder strb, Map<String, Object> paramValues) {
    ...
    BuildResult result =
        buildFromSelectClause(
            joinClause.getTableName(),
            joinClause.getSelectQuery(),
            joinClause.getAlias(),
            strb,
            paramValues);
  ...
  /**
   * Gets the string value or the transformed string value of obj. Checks to see if obj is a named
   * lazy parameter, in which case it will try to replace it will the named parameter from
   * paramValues. If the named parameter cannot be found, it's turned into a prepared param '?' and
   * recorded in the {@link BuildResult}
   *
   * @param obj The object to get the string value for
   * @param result Records the indices of any prepared params. This is an out parameter, and must
   *     not be null.
   * @param paramValues The map of named params to replacement objects.
   * @return Returns the string version of obj
   */
  private String getValueOrTxParamValue(
      Object obj, BuildResult result, Map<String, Object> paramValues) {
    String str = null;
    if (obj != null) {
      if (obj instanceof LazyNamedParam) {
        ...
        if (replacementVal != null) {
          ...
          // there's no replacement for the lazy value, so turn it into a prepared param, and record
          // it
          str = "?";
