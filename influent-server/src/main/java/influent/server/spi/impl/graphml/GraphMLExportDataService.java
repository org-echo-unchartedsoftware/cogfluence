...
public class GraphMLExportDataService implements ExportDataService {
  ...
  private static final String DEFAULT_STORE = "aperture.render";
  ...
  @Override
  public DocumentDescriptor exportToXMLDoc(JSONObject JSONData, String version)
      throws ConflictException, JSONException {
    ...
    try {
      ...
      throw new ResourceException(
          Status.SERVER_ERROR_INTERNAL, "Failure serializing graph data with JAXB", e);
    }
  ...
  public String exportToXML(JSONObject jsonData) throws JSONException {
    try {
      ...
      throw new ResourceException(
          Status.SERVER_ERROR_INTERNAL, "Failure converting graph data to UTF-8", e);
    }
  }

  private ByteArrayOutputStream convertJSONToGraphML(JSONObject jsonData, String version)
          ...
          if (file.get("UIType").equals("xfFile")) {
            ...
            try {
              final ContextReadWrite contextRW =
                  contextCache.getReadWrite(file.getString("xfId"), permits);
              for (FL_Cluster cluster : contextRW.getClusters()) {
                for (FL_Property property : cluster.getProperties()) {
                  ...
                  property.setRange(
                      SingletonRangeHelper.from(XML.escape((String) range.getValue())));
                }
              }
  ...
  /**
   * Add definition keys to a given GraphML. These keys specify how the node date should look in the
   * output graph, and hence are constant across all .xml files produced.
   *
   * @param graphML
   */
