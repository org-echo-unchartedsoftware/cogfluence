...
public class ChartHash {
  ...
  public ChartHash(String hash) {
    ...
    _focusContextId =
        (hashParts[2].compareToIgnoreCase("null") == 0 || hashParts[2].isEmpty())
            ? null
            : (hashParts[2]);
    _startDate = DateTimeParser.parse(hashParts[3]);
    _endDate = DateTimeParser.parse(hashParts[4]);
    _focusMaxDebitCredit =
        (hashParts[5].compareToIgnoreCase("null") == 0 || hashParts[5].isEmpty())
            ? null
            : Double.parseDouble(hashParts[5]);
    ...
    for (int i = 0; i < idParts.length; i++) {
      ...
      // Strip any versions
      idPart =
          idPart.indexOf(VERSION_SEPARATOR) != -1
              ? idPart.substring(0, idPart.indexOf(VERSION_SEPARATOR))
              : idPart;
    ...
    if (fidSectionIdx < -1) {
      String[] fidParts =
          hashParts[9]
              .substring(fidSectionIdx + FOCUS_SECTION.length() + SEPARATOR.length())
              .split("\\" + SEPARATOR);
      if (fidParts.length == 0) {
        ...
        for (int i = 0; i < fidParts.length; i++) {
          ...
          // Strip any versions
          fidPart =
              fidPart.indexOf(VERSION_SEPARATOR) != -1
                  ? fidPart.substring(0, fidPart.indexOf(VERSION_SEPARATOR))
                  : fidPart;
  ...
  private String _getVersion(String id, ContextRead contextRO) {
    ...
    if (contextRO != null && tId.getIdClass() == InfluentId.CLUSTER) {
      ...
      if (tId.getNativeId().startsWith("file")) {
        ...
        if (cluster != null) return VERSION_SEPARATOR + cluster.getVersion();
      }
    }
