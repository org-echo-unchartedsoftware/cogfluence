...
/**
 * This class handles reading in a graphml file, parses nodes and edges as desired, and re-saves the
 * data in tab-delimited format.
 *
 * <p>The class constructor takes in an argument map of key/values that is used to configure how the
 * data is parsed:
 *
 * <p>in -- Path and filename of graphML input file [required].
 *
 * <p>-n/eOut -- Path and filename of tab-delimited output file [required].
 *
 * <p>-longIDs -- [boolean, optional] If == true, then nodes will be assigned a unique Long number
 * ID, regardless of the node ID format in the original graphML file. Note, this ID convention is
 * needed for data processing with Spark's GraphX library. Default == false.
 *
 * <p>nAttr -- Node attributes to parse (attribute ID tags separated by commas) [optional]. Default
 * is to parse all node attributes.
 *
 * <p>eAttr -- Edge attributes to parse (attribute ID tags separated by commas) [optional]. Default
 * is to parse all edge attributes.
 *
 * <p>nCoordAttr -- Node attributes to use for node co-ordinates (separated by commas) [optional].
 * Default is NO co-ordinate data will be associated with a given node.
 *
 * <p>nCoordConvert -- Node co-ordinate conversion [optional, may be used with 'nCoordAttr' property
 * above] Choices are: zorder2xy (z-order to x-y), zorder2xyz (z-order to x,y,z -- note: z-axis data
 * will be discarded!) Default is no conversion
 */
public class GraphMLConverter {
  ...
  public GraphMLConverter(HashMap<String, String> argMap) {
    ...
    String stringTemp =
        argMap.get(
            "nAttr"); // nAttr, "Node attributes to parse (attribute ID tags separated by commas)"
    ...
    stringTemp =
        argMap.get(
            "eAttr"); // eAttr, "Edge attributes to parse (attribute ID tags separated by commas)."
    if (stringTemp != null) { // Default = parse all existing attributes.
    ...
    stringTemp =
        argMap.get(
            "nCoordAttr"); // nCoordAttr, "Node attributes to use for node coords (separated by
    // commas)"
    if (stringTemp != null) {
  ...
  public void parseGraphML() {
          ...
          new DefaultHandler() {
            ...
            public void startElement(
              ...
              if (qName.equalsIgnoreCase("key")) {
                ...
                if (attributes
                    .getValue("for")
                    .equalsIgnoreCase("node")) { // store all attribute ID's for nodes
                  ...
                  if (_nodeAttributes == null) {
                    ...
                    .getValue("for")
                    .equalsIgnoreCase("edge")) { // store all attribute ID's for edges
            ...
            public void endElement(String uri, String localName, String qName) throws SAXException {
              ...
              if (qName.equalsIgnoreCase("node")) {
                ...
                if (_nodeCoordAttr != null) {
                  ...
                  for (int n = 0; n < _nodeCoordAttr.size(); n++) {
                    String coordTemp =
                        nodeAttrValues.get(nodeAttrList.indexOf(_nodeCoordAttr.get(n)));
                    ...
                    if (_nodeCoordConvert.equals("zorder2xy")) {
                      Integer[] coordsTmp =
                          mortonXY(
                              (long)
                                  nodeData[
                                      0]); // assume z-order value has been parsed into coords[0]
                  ...
                  if (_bLongIDs) {
                    nodeData[2] =
                        (double)
                            _numNodes; // also save current _numNodes values as a unique Long ID for
                    // this node
                  }
                  ...
                  nodeData[2] =
                      (double)
                          _numNodes; // save current _numNodes values as a unique Long ID for this
                  // node
                  _nodemap.put(nodeID, nodeData);
                }
                      ...
                      if (!_nodeCoordAttr.contains(nodeAttrList.get(i))) {
                        _nodeBuffWriter.write(
                            "\t"
                                + escape(
                                    nodeAttrValues.get(
                                        i))); // write this attribute if not already written out as
                        // coords above
                      }
                ...
                if ((_numNodes % 100000L == 0)
                    && (_numNodes != 0)) { // print message every 100,000 iterations
                ...
                try {
                  if (_nodeCoordAttr != null) {
                    ...
                    if (_bLongIDs) {
                      // write out unique Long IDs for edge source and dest for edges instead of
                      // 'original' node IDs
                ...
                if ((_numEdges % 100000L == 0)
                    && (_numEdges != 0)) { // print message every 100,000 iterations
    ...
    try {
      ...
      _outBuffWriterReadme.write(
          "This readme file contains column labels for tab-delimited graph data in \n"
              + _nodesOut
              + "\n\n");
