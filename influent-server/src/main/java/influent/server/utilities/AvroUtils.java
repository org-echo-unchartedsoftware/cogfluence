...
public class AvroUtils {
   ...
   *
   * <p>Note: Just calling record.toString() doesn't encode union types losslessly (since it leaves
   * out their type).
   */
  public static String encodeJSON(SpecificRecordBase record) throws Exception {
    ...
    DatumWriter<SpecificRecord> datumWriter =
        new SpecificDatumWriter<SpecificRecord>(record.getSchema());
   ...
   *
   * <p>Note: This method will fail on strings created by calling record.toString() since that
   * doesn't encode union types losslessly.
   */
