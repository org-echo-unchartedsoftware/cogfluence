...
public class ChartImage extends BufferedImage {
  ...
  public void draw() {
    ...
    x = x + DOT_DIAMETER + 1;
    g2d.fillRect(
        x, centerline, (numBuckets * BAR_WIDTH) + ((numBuckets - 1) * SPACER) + (BAR_WIDTH * 2), 1);
    ...
    for (int i = 0; i < numBuckets; i++) {
      ...
      if (moneyScaled > 0.0) {
        ...
        g2d.fillRect(x, centerline - 1 - barHeight, BAR_WIDTH, barHeight);
      }
      ...
      if (data.getFocusCredits().get(i) > 0) {
        ...
        g2d.fillRect(x, (int) Math.round(centerline - 1 - barHeight), BAR_WIDTH, barHeight);
      }

      // if clipped top, mark it
      if (clipped) {
        g2d.setColor(COLORS.CAP_BAR.color);
        g2d.fillRect(x - 1, centerline - 1 - maxViewableBarHeight, BAR_WIDTH + 2, 1);
      }
      ...
      if (moneyScaled > 0.0) {
        ...
        // main bar below.
        g2d.fillRect(x, centerline + 2, BAR_WIDTH, barHeight);
      }
