...
public class DynamicClustering extends AbstractClusteringDataAccess implements FL_Clustering {
  ...
  public List<String> clusterEntities(
    ...
    try {
      ...
      Pair<List<FL_Entity>, List<FL_Cluster>> immutableChildren =
          getImmutableChildren(mutableClusters, sourceContextId);
      ...
      ClusterContext updatedContext =
          _clusterer.clusterEntities(newEntities, newClusters, targetContext);
  ...
  private List<FL_Entity> getImmutableChildEntities(List<FL_Cluster> immutableClusters) {
    ...
    for (FL_Cluster immutableCluster : immutableClusters) {
      entities.addAll(
          _entityAccess.getEntities(immutableCluster.getMembers(), FL_LevelOfDetail.SUMMARY));
    }
    return entities;
  }
    ...
    try {
      ...
      if (focusEntityIds == null || focusEntityIds.isEmpty()) {
        ...
        Map<String, List<FL_Link>> relatedLinks =
            EntityAggregatedLinks.getRelatedAggregatedLinks(
                entityIds,
                direction,
                date,
                _entityAccess,
        ...
        Map<String, List<FL_Link>> relatedLinks =
            EntityAggregatedLinks.getAggregatedLinks(
                entityIds,
                focusEntityIds,
                direction,
                date,
  ...
  public Map<String, List<FL_Link>> getTimeSeriesAggregation(
    ...
    Map<String, List<FL_Link>> links =
        _entityAccess.getTimeSeriesAggregation(leafList, fociiList, date);

    // Change the ids on the links, so they map to the original clusters passed in, not the
    // underlying leaf nodes.
  ...
  private List<FL_Cluster> filterExistingClusters(
      List<FL_Cluster> clusters, ClusterContext context) {
    // if context doesn't exist then all entities are new
  ...
  private Pair<List<FL_Entity>, List<FL_Cluster>> getImmutableChildren(
      List<FL_Cluster> clusters, String contextId) {
    Pair<List<FL_Entity>, List<FL_Cluster>> children =
        new Pair<List<FL_Entity>, List<FL_Cluster>>();
    ...
    try {
      ...
      if (contextRO != null) {
        for (FL_Cluster cluster : clusters) {
          ...
          List<String> immutableClusterIds =
              InfluentId.filterInfluentIds(cluster.getSubclusters(), InfluentId.ACCOUNT_OWNER);
          immutableClusterIds.addAll(
              InfluentId.filterInfluentIds(cluster.getSubclusters(), InfluentId.CLUSTER_SUMMARY));

          children.second.addAll(contextRO.getClusters(immutableClusterIds));

          Pair<List<FL_Entity>, List<FL_Cluster>> subChildren =
              getImmutableChildren(contextRO.getClusters(subClusterIds), contextId);
  ...
  public List<String> getLeafIds(List<String> ids, String context, boolean searchImmutableClusters) {
    ...
    if (searchImmutableClusters) {
      ...
      Map<String, Set<String>> summaryMemberIds =
          this.getClusterSummaryMembers(
              InfluentId.filterInfluentIds(ids, InfluentId.CLUSTER_SUMMARY));
    ...
    lids.addAll(
        getClusterLeafIds(
            InfluentId.filterInfluentIds(ids, InfluentId.CLUSTER),
            context,
            searchImmutableClusters));
