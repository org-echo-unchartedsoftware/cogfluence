...
public class SearchSolrHelper {
   ...
   */
  public static String toSolrQuery(
    ...
    for (Map.Entry<String, List<FL_PropertyMatchDescriptor>> entry : termMap.entrySet()) {
      ...
      final List<FL_PropertyMatchDescriptor> orsAnds =
          new ArrayList<FL_PropertyMatchDescriptor>(termsByType.size());
      ...
      queryFragment.append(
          _getTermsFragment(orsAnds, entry.getKey(), propertyDescriptors, fieldMapper));
      ...
      if (!nots.isEmpty()) {
        ...
        queryFragment.append(
            _getTermsFragment(nots, entry.getKey(), propertyDescriptors, fieldMapper));
      }
  ...
  private static String _getDescriptorFragment(
      String type,
      FL_PropertyMatchDescriptor descriptor,
      FL_PropertyDescriptors propertyDescriptors) {
    ...
    if (FL_Constraint.FUZZY_PARTIAL_OPTIONAL.equals(descriptor.getConstraint())
        || FL_Constraint.FUZZY_REQUIRED.equals(descriptor.getConstraint())) {
      s.append(":(");
      ...
      for (Object value : values) {
        ...
        for (int i = 0; i < tokens.length; i++) {
          ...
          s.append(tokens[i]);
          s.append("~");
          if (descriptor.getSimilarity() != null) s.append(descriptor.getSimilarity());
  ...
  private static String _getIdPairFragment(
    ...
    FL_PropertyMatchDescriptor fromEntityIDTerm =
        FL_PropertyMatchDescriptor.newBuilder(fromTerm)
                    ...
                    FL_TypeMapping.newBuilder()
                        ...
                        .setMemberKey(
                            PropertyDescriptorHelper.mapKey(
                                FL_RequiredPropertyKey.FROM.name(),
    ...
    FL_PropertyMatchDescriptor toEntityIDTerm =
        FL_PropertyMatchDescriptor.newBuilder(toTerm)
  ...
  private static String _getTermsFragment(
    ...
    // Test FROM propertyKey to see if ENTITY/LINKED keys for this term should be handled as a
    // transactional to/from pairs, or as a single IDs.
    boolean handleEntityKeyAsTransaction =
        PropertyDescriptorHelper.find(
                FL_RequiredPropertyKey.FROM.name(), propertyDescriptors.getProperties())
            != null;
    ...
    if (firstTerm.getConstraint().equals(FL_Constraint.OPTIONAL_EQUALS)
        || firstTerm.getConstraint().equals(FL_Constraint.FUZZY_PARTIAL_OPTIONAL)) {
      conjunction = " OR ";
    }
    ...
    for (int i = 0; i < terms.size(); i++) {
      ...
      if (handleEntityKeyAsTransaction
        ...
        if (fields != null) {
          final ArrayList<FL_PropertyMatchDescriptor> mirrors =
              new ArrayList<FL_PropertyMatchDescriptor>();
          ...
          if (!mirrors.isEmpty()) {
            ...
            for (int m = 1; m < mirrors.size(); m++) {
              ...
              queryFragment.append(
                  _getDescriptorFragment(type, mirrors.get(m), propertyDescriptors));
            }
            if (mirrors.size() > 1) {
              queryFragment.append(")");
            }
    ...
    if (handlePairedEntityKeys) {
      ...
      queryFragment.append("((");
      queryFragment.append(
          _getIdPairFragment(entityIDTerm, linkedIDTerm, type, propertyDescriptors, "AND"));
      queryFragment.append(") OR (");
      queryFragment.append(
          _getIdPairFragment(linkedIDTerm, entityIDTerm, type, propertyDescriptors, "AND"));
      queryFragment.append(")) ");
    }
    return queryFragment.toString();
  }
  ...
  /**
   * See org.apache.solr.client.solrj.util.ClientUtils See: {@link
   * org.apache.lucene.queryparser.classic queryparser syntax} for more information on Escaping
   * Special Characters
   */
