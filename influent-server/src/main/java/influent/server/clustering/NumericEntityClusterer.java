...
public class NumericEntityClusterer extends BaseEntityClusterer {
  ...
  private BaseClusterer createNumericClusterer(boolean kmeans) {
    ...
    clusterer.registerFeatureType(
        "num", MeanNumericVectorCentroid.class, new EuclideanDistance(1.0));

    return clusterer;
  }

  private ClusterContext secondStageClustering(
      Collection<FL_Entity> entities,
      Collection<FL_Cluster> immutableClusters,
  ...
  public ClusterContext firstStageClustering(
      Collection<FL_Entity> entities,
      Collection<FL_Cluster> immutableClusters,
      Collection<FL_Cluster> clusters,
      ClusterContext context) {
  ...
  public ClusterContext clusterEntities(
      ...
      Collection<FL_Cluster> clusters,
      ClusterContext context) {
    ClusterContext modifiedClusters =
        firstStageClustering(entities, immutableClusters, clusters, context);
    ...
    if (totalClusters > MAX_CLUSTER_SIZE) {
      modifiedClusters =
          secondStageClustering(entities, immutableClusters, newClusters, clusters, context);
    }
