...
public class GeneralEntityClusterer extends BaseEntityClusterer {
  ...
  private List<EntityClusterer> createClusterStages(Properties pMgr) throws Exception {
    ...
    try {
      String[] clusterFields =
          pMgr.getString(ClustererProperties.CLUSTER_FIELDS, "GEO:geo,TYPE:categorical,LABEL:label")
              .split(",");
      ...
      for (String field : clusterFields) {
        ...
        if (tokens.length > 1) {
          ...
          if (type.equalsIgnoreCase("categorical")) {
            ...
            // first group alphabetically - adjust alpha bucket size based upon max cluster size
            // property
            ...
            Object[] params = {
              clusterFactory, tagOrFieldName, geoCoder, GeoEntityClusterer.GEO_LEVEL.Continent
            };
            clusterer.init(params);
            clusterStages.add(clusterer);
            ...
            Object[] params2 = {
              clusterFactory, tagOrFieldName, geoCoder, GeoEntityClusterer.GEO_LEVEL.Region
            };
            clusterer.init(params2);
            clusterStages.add(clusterer);
            ...
            clusterer = new GeoEntityClusterer();
            Object[] params3 = {
              clusterFactory, tagOrFieldName, geoCoder, GeoEntityClusterer.GEO_LEVEL.Country
            };
            clusterer.init(params3);
            clusterStages.add(clusterer);
            ...
            clusterer = new GeoEntityClusterer();
            Object[] params4 = {
              clusterFactory, tagOrFieldName, geoCoder, GeoEntityClusterer.GEO_LEVEL.LatLon
            };
            ...
            log.error(
                "Invalid cluster field type specified for '{}' in clusterer.properties - verify the value of entity.clusterer.clusterfields",
                tagOrFieldName);
          }
        } else {
          log.error(
              "Invalid cluster field '{}' specified in clusterer.properties - verify the value of entity.clusterer.clusterfields",
              field);
        }
      }
    } catch (Exception e) {
  ...
  public ClusterContext clusterEntities(
    ...
    ClusterContext results =
        clusterStages.get(0).clusterEntities(entities, immutableClusters, mutableRoots, context);
    ...
    // keep track of a repeatable stages results to determine when it no longer is successful at
    // clustering entities
    ...
    while (!clustersToSplit.isEmpty()) {
      ...
      for (FL_Cluster cluster : clustersToSplit) {
        ...
        results =
            clusterer.clusterEntities(
                getChildEntities(cluster, context, false),
                getSubClusters(cluster, context),
                context);
      ...
      if (isCompletedStage(currentStage, prevResultCount, stageResults.size())) {
        currentStage =
            Math.min(
                clusterStages.size() - 1,
                ++currentStage); // iterate through stages and stay on last stage until done
        prevResultCount = 0; // reset prev result count since we are in a new stage
      } else {
        prevResultCount =
            stageResults
                .size(); // still in the current stage so update the results produced this iteration
      }
    }
  ...
  private boolean isRepeatableStage(int currentStage) {
    EntityClusterer clusterer = clusterStages.get(currentStage);
    return (clusterer instanceof NumericEntityClusterer
        || clusterer instanceof LabelEntityClusterer);
  }
  ...
  private boolean isCandidate(FL_Cluster cluster) {
    ...
    int numMutables =
        InfluentId.filterInfluentIds(cluster.getSubclusters(), InfluentId.CLUSTER).size();
  ...
  private void findClustersToSplit(
      Map<String, FL_Cluster> candidates, Collection<FL_Cluster> splitQueue) {
  ...
  private void updateClusterReferences(FL_Cluster cluster, Map<String, FL_Cluster> children) {
    ...
    List<String> mutableSubClusters =
        InfluentId.filterInfluentIds(cluster.getSubclusters(), InfluentId.CLUSTER);
    Set<String> uniqueIds = new LinkedHashSet<String>(mutableSubClusters);
    uniqueIds.addAll(childClusterIds);
    List<String> sortedSubClusters = new ArrayList<String>(uniqueIds);
    Collections.sort(sortedSubClusters);
    cluster.setSubclusters(sortedSubClusters);
  }
}
