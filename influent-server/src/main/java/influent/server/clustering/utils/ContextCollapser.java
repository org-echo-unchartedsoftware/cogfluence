...
public class ContextCollapser {
  ...
  // Returns a simplified set of clusters where all the chain-links have been removed (a chain-link
  // being a
  // subcluster that contains only 1 child (1 subcluster or 1 member)
  public static Pair<Collection<FL_Entity>, Collection<FL_Cluster>> collapse(
      ContextRead context, boolean leaveRoots, Map<String, String> output_redirect) {

    Pair<Collection<FL_Entity>, Collection<FL_Cluster>> output =
        new Pair<Collection<FL_Entity>, Collection<FL_Cluster>>();
    ...
    for (FL_Cluster cluster : clusterList) { // prune any and all chain-links
      if (isChain(cluster)) {
        ...
        if (cluster.getParent() == null) {
          ...
          // remove the cluster id from the parent subcluster list and add the subcluster id from
          // above
        ...
        // first case, the cluster is a root cluster, meaning the entity is a 'result' - entity with
        // no containing cluster.
        if (cluster.getParent() == null) {
          if (!leaveRoots) {
            results.remove(cluster.getUid());
          ...
          if (parentCluster == null) {
            s_logger.error(
                "Cannot locate parent " + cluster.getParent() + " for cluster " + cluster.getUid());
          } else {
            // remove the cluster id from the parent subcluster list and add the entity id from
            // above
            ClusterHelper.removeSubCluster(parentCluster, cluster);
