...
public abstract class BaseEntityClusterer implements EntityClusterer {
  ...
  protected List<FL_Entity> getChildEntities(
      FL_Cluster cluster, ClusterContext context, boolean recurse) {
  ...
  public static boolean isImmutableCluster(String clusterId) {
    return (InfluentId.hasIdClass(clusterId, InfluentId.ACCOUNT_OWNER)
        || InfluentId.hasIdClass(clusterId, InfluentId.CLUSTER_SUMMARY));
  }

  public static boolean isImmutableCluster(FL_Cluster cluster) {
    String id = cluster.getUid();
    return (InfluentId.hasIdClass(id, InfluentId.ACCOUNT_OWNER)
        || InfluentId.hasIdClass(id, InfluentId.CLUSTER_SUMMARY));
  }
  ...
  protected Map<String, FL_Cluster> mergeClusterBuckets(
      Map<String, List<FL_Cluster>> srcBuckets, Map<String, FL_Cluster> dstBuckets) {
    Map<String, FL_Cluster> modifiedClusters = new HashMap<String, FL_Cluster>();

    // merge buckets that have the same bucket id or create a new cluster if no previous bucket
    // exists
    // Sort bucket keys to ensure deterministic cluster ID assignment
    List<String> sortedBucketIds = new ArrayList<String>(srcBuckets.keySet());
    Collections.sort(sortedBucketIds);
    for (String bucketId : sortedBucketIds) {
  ...
  protected Map<String, FL_Cluster> mergeEntityBuckets(
      Map<String, List<FL_Entity>> srcBuckets, Map<String, FL_Cluster> dstBuckets) {
    Map<String, FL_Cluster> modifiedClusters = new HashMap<String, FL_Cluster>();
  ...
  protected Collection<FL_Cluster> findNewClusters(
      Map<String, FL_Cluster> modified, Collection<FL_Cluster> existing) {
  ...
  protected Map<String, FL_Cluster> retrieveClusterBuckets(
      Collection<FL_Cluster> clusters, ClusterContext context) {
  ...
  protected Map<String, List<FL_Cluster>> bucketClusters(
      Collection<FL_Cluster> clusters, ClusterContext context) {
  ...
  public ClusterContext clusterEntities(
      Collection<FL_Entity> entities, Collection<FL_Cluster> clusters, ClusterContext context) {
