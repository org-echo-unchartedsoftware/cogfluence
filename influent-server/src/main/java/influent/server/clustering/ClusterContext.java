...
public class ClusterContext implements Serializable {
  ...
  private void writeObject(java.io.ObjectOutputStream out) throws IOException {
    ...
    final DatumWriter<Map<String, FL_Cluster>> cwriter =
        new SpecificDatumWriter<Map<String, FL_Cluster>>(CLUSTER_MAP_SCHEMA);
    cwriter.write(roots, encoder);
    cwriter.write(clusters, encoder);

    final DatumWriter<Map<String, FL_Entity>> ewriter =
        new SpecificDatumWriter<Map<String, FL_Entity>>(ENTITY_MAP_SCHEMA);
  ...
  private void readObject(java.io.ObjectInputStream in) throws IOException, ClassNotFoundException {
    ...
    final DatumReader<Map<String, FL_Cluster>> creader =
        new SpecificDatumReader<Map<String, FL_Cluster>>(CLUSTER_MAP_SCHEMA);
    roots = creader.read(roots, decoder);
    clusters = creader.read(clusters, decoder);

    final DatumReader<Map<String, FL_Entity>> ereader =
        new SpecificDatumReader<Map<String, FL_Entity>>(ENTITY_MAP_SCHEMA);
