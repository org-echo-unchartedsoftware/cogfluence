...
public abstract class SolrBaseSearchIterator implements Iterator<FL_SearchResult> {
  ...
  /**
   * Returns true if there are more results to fetch but we've already iterated over the current
   * set.
   *
   * @return
   */
  ...
  protected List<FL_Property> getPropertiesFromDocument(
      SolrDocument sd, String type, List<FL_PropertyDescriptor> descriptors) {
    ...
    for (FL_PropertyDescriptor prop : descriptors) {
      ...
      boolean isHidden =
          prop.getLevelOfDetail().equals(FL_LevelOfDetail.HIDDEN)
      ...
      if (dataList.size() > 1) {
        range =
            FL_ListRange.newBuilder().setType(prop.getPropertyType()).setValues(dataList).build();
      } else {
        range =
      ...
      props.add(
          new PropertyHelper(
              prop.getKey(), prop.getFriendlyText(), null, null, prop.getTags(), isHidden, range));
    }
  ...
  protected List<Object> getCompositeFieldValues(
      Map<String, Collection<Object>> values, String key, String type, String fieldName) {
  ...
  protected List<Object> getPropertyValuesFromDocument(
      Map<String, Collection<Object>> values, FL_PropertyDescriptor pd, String type) {
  ...
  protected String getTypeFromDocument(
      SolrDocument sd, FL_PropertyDescriptors propertyDescriptors) {
    String type = (String) sd.getFieldValue(FL_RequiredPropertyKey.TYPE.name());
    ...
    if (type == null) {
      if (propertyDescriptors.getTypes().size() == 1) {
        ...
        throw new RuntimeException(
            "Solr results must include a type field if there is more than one possibility.");
      }
    }
