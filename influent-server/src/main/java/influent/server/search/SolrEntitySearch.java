...
public class SolrEntitySearch extends DataViewEntitySearch {
  ...
  public FL_SearchResults search(
    ...
    // Form the query
    String searchStr =
        SearchSolrHelper.toSolrQuery(termMap, getDescriptors(), getPropertyFieldProvider());
    ...
    orderBy =
        PropertyDescriptorHelper.mapOrderBy(
            orderBy, getDescriptors().getProperties(), termMap.keySet());

    if (orderBy != null) {
      for (FL_OrderBy ob : orderBy) {
        String key =
            (ob.getPropertyKey().equals(FL_ReservedPropertyKey.MATCH.name()))
                ? "score"
                : ob.getPropertyKey();
  ...
  protected SolrEntitySearchIterator buildSolrEntitySearchIterator(
    ...
    return new SolrEntitySearchIterator(
        namespaceHandler,
        solr,
        query,
        config,
