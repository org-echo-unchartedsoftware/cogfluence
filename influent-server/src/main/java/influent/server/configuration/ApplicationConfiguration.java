...
public class ApplicationConfiguration implements PropertyField.Provider {
  ...
  public static enum SystemColumnType {
    STRING,
    DOUBLE,
    FLOAT,
    INTEGER,
  ...
  public boolean hasMultipleEntityTypes() {
    return _entityDescriptors.getTypes().size() > 1;
  }

  public boolean hasMultipleLinkTypes() {
    return _linkDescriptors.getTypes().size() > 1;
  }
      ...
      if (!fXmlFile.exists()) {
        ...
        if (fileURL == null) {
          throw new IOException(
              "could not find file " + "./conf/" + filename + " or " + "/" + filename);
        }
  ...
  private static void _populateDescriptors(String rootTagName, FL_PropertyDescriptors descriptors) {
    ...
    Element docRoot =
        _getDocumentRoot(
            _config.getString(
                "influent.midtier.property.configfile", DEFAULT_PROPERTY_DESCRIPTOR_FILE));
    ...
    for (Element t : _getElementsFromTagName(root, "type")) {
      ...
      for (Element group : _getElementsFromTagName(root, "group")) {
        ...
        if (group.getAttribute("key").equals(groupId)) {
          ...
          if (groupName == null || groupName.isEmpty()) groupName = groupId;
          isExclusive = group.getAttribute("exclusive").equals("true");
          break;
        }
      }

      FL_TypeDescriptor typeDesc =
          FL_TypeDescriptor.newBuilder()
    ...
    for (Element p : _getElementsFromTagName(root, "property")) {
      ...
      FL_PropertyDescriptor propDesc =
      ...
      if (memberOfElements.size() != 0) {
        ...
        // if none specified interpret as applying to all, with the same key unless otherwise
        // specified
      } else {
        ...
        for (FL_TypeDescriptor type : typeList) {
          FL_TypeMapping typeMapping =
              FL_TypeMapping.newBuilder().setType(type.getKey()).setMemberKey(memberKey).build();
      ...
      if (rootName.equalsIgnoreCase(ENTITY_ROOT)
          && propDesc.getKey().equalsIgnoreCase(FL_RequiredPropertyKey.ID.name())) {
        propertyList.add(
            FL_PropertyDescriptor.newBuilder()
                ...
                .build());
      } else if (rootName.equalsIgnoreCase(LINK_ROOT)
          && propDesc.getKey().equalsIgnoreCase(FL_RequiredPropertyKey.FROM.name())) {
        List<FL_TypeMapping> emptyTypeMapping = new ArrayList<FL_TypeMapping>();
        for (FL_TypeDescriptor type : typeList) {
          FL_TypeMapping typeMapping =
              FL_TypeMapping.newBuilder().setType(type.getKey()).setMemberKey("").build();
        ...
        propertyList.add(
          ...
          && propDesc.getKey().equalsIgnoreCase(FL_RequiredPropertyKey.TO.name())) {
        List<FL_TypeMapping> emptyTypeMapping = new ArrayList<FL_TypeMapping>();
        for (FL_TypeDescriptor type : typeList) {
          FL_TypeMapping typeMapping =
              FL_TypeMapping.newBuilder().setType(type.getKey()).setMemberKey("").build();
  ...
  private static void _populateApplicationSchema() {
    Element docRoot =
        _getDocumentRoot(
            _config.getString(
                "influent.midtier.database.configfile", DEFAULT_SYSTEM_CONFIGURATION_FILE));
