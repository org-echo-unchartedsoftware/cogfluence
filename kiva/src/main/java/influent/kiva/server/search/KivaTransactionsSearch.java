...
public class KivaTransactionsSearch extends DataViewLinkSearch implements FL_LinkSearch {
  ...
  protected FL_Link createLink(
    ...
    if (fromProp != null) {
      try {
        List<FL_Entity> entityResults =
            service.getEntities(Collections.singletonList(from), FL_LevelOfDetail.SUMMARY);
            ...
            if (entity.getUid().equalsIgnoreCase(from)) {
              fromProp.setRange(
                  SingletonRangeHelper.from(
                      EntityHelper.getFirstPropertyByTag(entity, FL_PropertyTag.NAME)
                              .getValue()
                              .toString()
                          + " ("
                          + PropertyHelper.getValue(fromProp)
                          + ")"));
            }
          }
        }
    ...
    if (toProp != null) {
      try {
        List<FL_Entity> entityResults =
            service.getEntities(Collections.singletonList(to), FL_LevelOfDetail.SUMMARY);
        if (entityResults != null && entityResults.size() > 0) {
          for (FL_Entity entity : entityResults) {
              ...
              toProp.setRange(
                  SingletonRangeHelper.from(
                      EntityHelper.getFirstPropertyByTag(entity, FL_PropertyTag.NAME)
                              ...
                              .toString()
                          + " ("
                          + PropertyHelper.getValue(toProp)
                          + ")"));
            }
          }
        }
