...
/**
 * Search result object for iterating through the results of a search. Does paging, etc, behind the
 * scenes.
 *
 * @author msavigny
 */
public class KivaEntitySearchIterator extends SolrEntitySearchIterator {
  ...
  protected FL_Entity buildResultFromDocument(SolrDocument sd) {
    ...
    String uid =
        sd.getFieldValue(
                PropertyDescriptorHelper.mapKey(
                    FL_RequiredPropertyKey.ID.name(), entityDescriptors.getProperties(), type))
            .toString();
    ...
    List<FL_Property> props =
        getPropertiesFromDocument(sd, type, entityDescriptors.getProperties());
    ...
    char entityClass =
        type.equals(KivaTypes.TYPE_PARTNER) ? InfluentId.ACCOUNT_OWNER : InfluentId.ACCOUNT;
  ...
  protected void _appendPartnerProperties(List<FL_Property> props) {
    final FL_Property delinquency =
        PropertyHelper.getPropertyByKey(props, "partner_delinquencyRate");
