...
public class ClusterEntities implements Serializable {
  ...
  public ClusterEntities(
      Map<String, SchemaField> schemaMap,
      String[][] clusterOrder,
      String inputDir,
      String outputDir,
      int minclustersize) {
            ...
            new PairFunction<Tuple2<String, Instance>, String, Instance>() {
              ...
              public Tuple2<String, Instance> call(Tuple2<String, Instance> item) throws Exception {
                ...
                return new Tuple2<String, Instance>(
                    clusterKey + "-" + category.replaceAll("[^\\w]", ""), inst);
              }
            });
    return results;
  }

  private JavaPairRDD<String, Instance> clusterByCountryCode(
                ...
                if (inst.containsFeature(fieldName, fieldName)) {
                  ...
                  countrycode =
                      f.getFreqTable()
                          ...
                          .next()
                          .getFeature()
                          .getName(); // ((StringFeature)inst.getFeature(fieldName,
                  // fieldName).iterator().next()).getValue();
  ...
  private JavaPairRDD<String, Instance> clusterByLatLon(
      JavaSparkContext sc,
      JavaPairRDD<String, Instance> instances,
      String fieldName,
      final String clusterKey) {
    ...
    KMeansClusterer clusterer =
        new KMeansClusterer(K, MAX_KMEANS_ITERATIONS, KMEANS_STOPPING_THRESHOLD);

    clusterer.registerFeatureType(
        fieldName, fieldName, FastGeoSpatialCentroid.class, new HaversineDistance(1.0));
    SparkClusterResult result = clusterer.doCluster(ds);

    return result.getRDD();
  }

  private JavaPairRDD<String, Instance> clusterByLabel(
      JavaSparkContext sc,
      JavaPairRDD<String, Instance> instances,
      String fieldName,
      final String clusterKey) {
    ...
    clusterer.registerFeatureType(
        fieldName, fieldName, BagOfWordsCentroid.class, new EditDistance(1.0));
    SparkClusterResult result = clusterer.doCluster(ds);

    return result.getRDD();
  }

  private JavaPairRDD<String, Instance> clusterByNumeric(
      JavaSparkContext sc,
      JavaPairRDD<String, Instance> instances,
      String fieldName,
      final String clusterKey) {
    ...
    clusterer.registerFeatureType(
        fieldName, fieldName, MeanNumericVectorCentroid.class, new EuclideanDistance(1.0));
    SparkClusterResult result = clusterer.doCluster(ds);

    return result.getRDD();
  }

  private JavaPairRDD<String, Instance> clusterByFeatures(
      JavaSparkContext sc,
      JavaPairRDD<String, Instance> instances,
      String[] fieldNames,
    ...
    for (int i = 0; i < fieldNames.length; i++) {
      switch (fieldTypes[i]) {
        ...
        case LABEL:
          clusterer.registerFeatureType(
              fieldNames[i], fieldNames[i], BagOfWordsCentroid.class, new EditDistance(1.0));
          break;
        case GEO:
          clusterer.registerFeatureType(
              fieldNames[i],
  ...
  public void doCluster() {
    ...
    String jobName =
        "influent clustering"; // name that is shown in job monitoring consoles when running in
    // distributed mode
    ...
    //
    // Top down Divisive hierarchical clustering using binning and k-means clustering - each stage
    // clusters by a distinct feature
    //
    while (true) {
      ...
      for (String clusterSet : clusterSets) {
        SparkDataSet ds = new SparkDataSet(sc);
        ds.load(
            clusterSet, new EntityInstanceParser(new ArrayList<SchemaField>(schemaMap.values())));
        //				ds.getRDD().cache();
        ...
        for (final String clusterKey : clusterKeys) {
          ...
          if (count > minClusterSize) {
            ...
            // if the cluster was successfully split during this iteration then save result and
            // schedule results for further split
            // otherwise, re-schedule the cluster for splitting in the next iteration
            long subClusterCount = results.keys().distinct().count();

            log.error(
                "* Sub-clustering time: " + (System.currentTimeMillis() - clusterStart) + " ms");

            if (isValidClustering(results, subClusterCount)) {
              log.error(
                  "* Cluster successfully split on field: "
                      + fieldsToString(fields)
                      + ": scheduling sub-clusters for splitting");
              results.saveAsTextFile(clusterOutput);
              newClusters = true;
              clustersToSplit.add(clusterOutput);
              clusterCount += (int) subClusterCount;
            } else {
              log.error(
                  "* Cluster couldn't be split on field: "
                      + fieldsToString(fields)
                      + ": scheduling for next phase clustering");
            ...
            log.error(
                "* Skipping cluster with key: " + clusterKey + ": size is <" + minClusterSize);
          }
        }
      }
      ...
      if (fields.length == 1) {
        ...
        switch (types[0]) {
          case CC:
            ...
            if (curGeoLvl == null)
              curField++; // we are done with binning on country code - move to next field
            fieldIteration = 0;
            break;
          case GEO:
          case NUMBER:
    ...
    System.out.println(
        "Total Hiearchical Clustering time (ms): " + (System.currentTimeMillis() - start));
  }

  private static void printUsageMsg() {
    System.out.println(
        "Please specify an input directory of pre processed cluster input files and output path.");
    System.out.println(
        "USAGE: EntityClusteringJob --schema=\"<comma list of featureName:<ID | GEO | LABEL | CC | NUMBER | CATEGORY>>\" --clusterorder=\"<comma list of featureNames specified in schema>\" --inputdir=\"<INPUT DIR>\" --ouputdir=\"<OUTPUT DIR>\"");
    System.out.println(
        "OPTIONAL arguments: --minclustersize=\"INTEGER\"  determines the stopping criterion for further splitting a cluster");
    System.out.println(
        "EXAMPLE:  EntityClusteringJob --schema=\"entityType:CATEGORY,location:GEO,name:LABEL,avgTrns:NUMBER\" --clusterorder=\"entityType,location,avgTrns,name\" --inputdir=\"instances\" --outputdir=\"clusters\"");
  }
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      if (inputDir == null || outputDir == null || schemaMap == null || clusterOrder == null)
