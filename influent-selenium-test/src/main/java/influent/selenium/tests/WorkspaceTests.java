...
public class WorkspaceTests extends BrowserParameterizedTest {
  ...
  public WorkspaceTests(BROWSER browser) {
    super(browser);
  }
   ...
   *
   * @param expectedTitle
   * @param expectedRoute The route should be anything after the '#'
   */
  private void assertRoute(String expectedTitle, String expectedRoute) {
    ...
    Assert.assertTrue(
        "Current path doesn't start with the startURL: " + startURL, curPath.startsWith(startURL));
    ...
    if (routeLoc >= 0) {
      ...
      Assert.assertTrue(
          "The route seems to have more of a path before the '#': " + curPath, routeLoc <= 1);
      ...
      Assert.assertTrue(
          "The current path should be at the root but instead we're at: " + curPath,
          curPath.length() == 0 || curPath.equals("/"));
    }
  }
  ...
  public void urlRoutingTest() {
    ...
    assertEquals(
        true,
        SeleniumUtils.hasClass(
            driver.findElement(By.id("infWorkspaceTabContentAccounts")), "active"));
    assertEquals(
        false,
        SeleniumUtils.hasClass(
            driver.findElement(By.id("infWorkspaceTabContentSummary")), "active"));
