...
public class BrowserParameterizedTest {
  ...
  public static Collection params() {
    return Arrays.asList(new Object[][] {{BROWSER.FIREFOX}, {BROWSER.CHROME}});
  }
  ...
  public static String getSystemStartURL() throws RuntimeException {
    ...
    try {
      uri = new URI(webhost);
      if (uri.getHost() == null)
        throw new Exception("Invalid webhost. Host or scheme is missing: '" + webhost + "'");

      if (uri.getPath().length() == 0) {
        if (!webapp.startsWith("/")) webapp = "/" + webapp;
        uri = uri.resolve(webapp);
      }
      startURL = uri.toASCIIString();
    } catch (Exception e) {
      throw new RuntimeException(
          "There was a problem setting up the webhost - app: '" + webhost + "' - '" + webapp + "'",
          e);
    }
    return startURL;
  }
  ...
  private void initializeRemote(BROWSER browser, URI host) throws MalformedURLException {
    switch (browser) {
      case FIREFOX:
        this.driver = new RemoteWebDriver(host.toURL(), new DesiredCapabilities());
        break;
      case CHROME:
      ...
      case IE:
        ...
        System.setProperty(
            "webdriver.ie.driver", System.getProperty("user.dir") + "\\IEDriverServer.exe");
        this.driver = new RemoteWebDriver(host.toURL(), new DesiredCapabilities());
        break;
      default:
        this.driver = null;
