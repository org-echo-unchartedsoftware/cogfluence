...
public class TransactionsTableTest extends BrowserParameterizedTest {
  ...
  public TransactionsTableTest(BROWSER browser) {
    super(browser);
  }
  ;
  ...
  private void getFlowViewResults(String query, int numCards, boolean expand) {
        ...
        (new WebDriverWait(driver, Duration.ofSeconds(120)))
            .until(
                ExpectedConditions.presenceOfElementLocated(
                    By.cssSelector(".infSearchResultCounts")));

    element =
        (new WebDriverWait(driver, Duration.ofSeconds(120)))
            .until(
                ExpectedConditions.presenceOfElementLocated(By.cssSelector(".infOperationsBar")));
    ...
    List<WebElement> dataIdElements =
        driver.findElements(By.cssSelector(".selectSingleResult input"));
  ...
  public void tableNoResultsTest() {
    ...
    element =
        (new WebDriverWait(driver, Duration.ofSeconds(120)))
            .until(
                ExpectedConditions.presenceOfElementLocated(
                    By.id("transactions-table-pagination-label")));
  ...
  public void tableMultiResultsOnePageTest() {
    ...
    element =
        (new WebDriverWait(driver, Duration.ofSeconds(120)))
            .until(
                ExpectedConditions.presenceOfElementLocated(By.className("simpleSearchSummary")));
    assertEquals("Showing 1 - 5 of 5 results", element.getText());
  }

  @Override
  public String toString() {
    return "TransactionsTableTest []";
  }
  ...
  public void tableMultiResultsMultiPageTest() {
    ...
    WebElement element =
        (new WebDriverWait(driver, Duration.ofSeconds(120)))
            .until(
                ExpectedConditions.presenceOfElementLocated(
                    By.cssSelector("[title='branch right to outflowing destinations']")));
    ...
    // Wait a few seconds for new table to load. We can't use
    // ExpectedConditions.presenceOfElementLocated since they are already present
    try {
      Thread.sleep(2000);
    } catch (Exception e) {
    }
    ...
    for (WebElement webElement : elements) {
      if (webElement.getAttribute("class") != null
          && webElement.getAttribute("class").length() >= 21
    ...
    assertEquals(
        "31 2012-11-30 status: paid; repayed: 181.65 (in PEN) $68.12 -", element.getText());
