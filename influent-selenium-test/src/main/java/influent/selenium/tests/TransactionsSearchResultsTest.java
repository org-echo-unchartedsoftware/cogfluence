...
public class TransactionsSearchResultsTest extends BrowserParameterizedTest {
   ...
   * @param expectedResults An array of expected search results. Each search result should be in the
   *     form:<br>
   *     { int index, String date, String to, String from }
   */
  private void validateSearchResults(
      String query, int expectedNumResults, Object expectedResults[][]) {
    List<WebElement> elements =
        driver.findElements(By.cssSelector("#influent-filterbar-search-input"));
        ...
        (new WebDriverWait(driver, 120))
            .until(
                ExpectedConditions.presenceOfElementLocated(
                    By.cssSelector(".simpleSearchSummary")));
    assertEquals(
        "Showing 1 - "
            + Math.min(expectedNumResults, MAX_NUM_RESULTS_PER_PAGE)
            + " of "
            + expectedNumResults
            + " results",
        element.getText());
    ...
    for (Object[] expectedResult : expectedResults) {
      ...
      Assert.assertNotNull(
          "Couldn't find search result " + resultNum + " for query: '" + query + "'", searchResult);
  ...
  private void assertSearchResult(String eDate, String eTo, String eFrom, WebElement searchResult) {
    ...
    e =
        searchResult.findElement(
            By.cssSelector(
                ".simpleSearchResultText .simpleSearchResultFieldValue span:nth-child(1)"));
    ...
    e =
        searchResult.findElement(
            By.cssSelector(
                ".simpleSearchResultText .simpleSearchResultFieldValue span:nth-child(3)"));
  ...
  private int gotoSearchResultsPageFor(int index) {
    WebElement selectedPageButton =
        resultsRoot.findElement(By.cssSelector(".searchPageElementHighlighted"));
    ...
    boolean isOnPage =
        index >= curPageNum * MAX_NUM_RESULTS_PER_PAGE
    ...
    WebElement pageButton =
        resultsRoot.findElement(By.cssSelector("span[searchpage='" + pageNum + "']"));
    pageButton.click();
  ...
  private WebElement getSearchResult(int index) {
    ...
    if (startIndex >= 0) {
      ...
      element =
          resultsRoot.findElement(
              (By.cssSelector(".simpleSearchResult:nth-child(" + (resultOffset + 1) + ")")));
    }
  ...
  public void testMultiAccountQuery() {
    ...
    // some random expected results from the query. This also picks some of the fence post indices
    // to test out different pages
    Object expectedResults[][] = {
    ...
    validateSearchResults(
        "INFLUENT_ID:a.null.b146773 INFLUENT_ID:a.null.b146772", 22, expectedResults);
  }
}
